...
const parseMultipartFormData = async (event) => {
    ...
    for (const part of parts) {
        // Handle file upload - Changed 'pdfFile' to 'pdf' to match frontend
        const fileMatch = part.match(/Content-Disposition: form-data; name="([^"]+)"; filename="([^"]+)"\r\nContent-Type: ([^\r\n]+)\r\n\r\n([\s\S]*?)(?:\r\n|$)/);
        if (fileMatch && fileMatch[1] === 'pdf') { // Changed from 'pdfFile' to 'pdf'
            const fileBuffer = Buffer.from(fileMatch[4], 'binary');
...
// Update generateSlideContent to include PDF content
async function generateSlideContent(topic, totalSlides, bibleVersion, pdfContent = null) {
    console.log('[INFO] Starting slide generation:', { topic, totalSlides, hasPDF: !!pdfContent });
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const slideContent = await generateSlideContent(topic, slideCount, bibleVersion, pdfContent);
